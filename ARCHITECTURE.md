...
- **Encryption**: [sops](https://github.com/getsops/sops) via
  ...
  the underlying crypto backend (sops encrypts a per-file data key to each age recipient; the file
...
- **Recipients**: declared in `nix-secrets`'s own `.sops.yaml`, a list of `key_groups` selected by
  ...
  identities under `users`-equivalent groups, per-host `*<host>_ssh` anchors, ...) are defined once
  as YAML anchors and referenced from multiple rules — check for an existing anchor/rule before
  inventing a new one. A fleet-wide catch-all `path_regex: .*` rule stays last (`sops` uses
  first-matching-rule-wins) as a safety net for any file added without its own rule.
- **Identities**: an offline, non-hardware `alberth` recovery key plus several backup YubiKeys
  (cached touch policy — one touch valid for 15s, PIN required once per session, via
  `age-plugin-yubikey`) for interactive/local decryption, plus **each host's own SSH host key**
  (`/etc/ssh/ssh_host_ed25519_key`, already present on every host since `services.openssh.enable`
  is set fleet-wide) converted to an X25519 age identity internally by sops-nix's
  `sops-install-secrets` at activation time — the recipient side needs that same conversion done
  ahead of time via `ssh-to-age -i <host>.pub` to get the matching `age1...` string for `.sops.yaml`
  (**not** the raw `ssh-ed25519 ...` string — see `nix-secrets/.sops.yaml`'s header comment for the
  full reasoning). No dedicated host identity file or generation step exists anymore: the SSH host
...
sequenceDiagram
    ...
    Dev->>Secrets: 1. Add/confirm a .sops.yaml rule covering the file
    Dev->>Secrets: 2. sops <file> (touch YubiKey; --input/output-type binary for keytabs)
    Secrets-->>Dev: <file> encrypted to every recipient in its matching rule
    Dev->>Nixie: 3. Wire sops.secrets.<name> (or nixie.krb5.keytabFile)
    ...
    Host->>Host: sops-install-secrets decrypts with the host's SSH key (sops.age.sshKeyPaths)
    Host-->>Host: plaintext at config.sops.secrets.<name>.path (/run/secrets/<name> by default)
```
...
Unlike the old per-host age key, there's no on-host generation step: the host's SSH host key
(present the moment `services.openssh.enable` activates, which every real `nixie` host sets) is
already usable as a decryption identity. To grant a host access to existing secrets:
...
3. `sops updatekeys <file>` for each affected file (touch the YubiKey once per file) — this
   ...
   so it's fast and doesn't require decrypting/re-encrypting the payload.
4. Commit and push.

The same `sops updatekeys` step is how a recipient is **removed**, too: delete its key from
`.sops.yaml`'s `key_groups`/anchor, then `sops updatekeys <file>`. This only changes who can
